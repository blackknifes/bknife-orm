...
public class TableMapperProxy<T> implements TableMapper<T> {
    private SqlAssemble assemble; // 组装器
    private DataSource dataSource; // 数据源
    private SqlTableInfo<T> tableInfo;

    public static <T> TableMapperProxy<T> create(SqlAssemble assemble, DataSource dataSource, SqlTableInfo<T> tableInfo)
            throws Exception {
        return new TableMapperProxy<>(assemble, dataSource, tableInfo);
    }

    private TableMapperProxy(SqlAssemble assemble, DataSource dataSource, SqlTableInfo<T> tableInfo) throws Exception {
